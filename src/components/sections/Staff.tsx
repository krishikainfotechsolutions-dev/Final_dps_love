          ...
          {active && (
            ...
            >
              <motion.div
                ...
                className="bg-card rounded-3xl overflow-hidden max-w-md w-full shadow-elegant max-h-[90vh] flex flex-col"
              >
                {/* Photo section — portrait ratio so full face/body is visible */}
                <div className="relative aspect-[3/4] w-full flex-shrink-0">
                  <img
                    ...
                    style={{
                      ...
                      objectPosition: active._imgPosition ?? 'center top',
                      transform: active._imgZoom && active._imgZoom !== 1 ? `scale(${active._imgZoom})` : undefined,
                      transformOrigin: active._imgPosition ?? 'center top',
                    }}
                  />
                ...
                {/* Info section — increased padding + min height */}
                <div className="p-6 min-h-[160px] flex flex-col justify-between overflow-y-auto">
                  <div>
                    <p className="text-sm text-muted-foreground leading-relaxed">{active.bio}</p>
                    {active.subjects && active.subjects.length > 0 && (
                      <>
                        <p className="mt-5 mb-2 text-[10px] uppercase tracking-widest font-semibold text-foreground/60">Subjects</p>
                        <div className="flex flex-wrap gap-2">
                          {active.subjects.map((sub: string) => (
                            <span key={sub} className="text-[11px] px-3 py-1.5 rounded-full bg-primary/10 text-primary border border-primary/20 font-medium">{sub}</span>
                          ))}
                        </div>
                      </>
                    )}
