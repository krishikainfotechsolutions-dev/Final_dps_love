...
const emptyForm: Record<Section, any> = {
  ...
  gallery: { title: "", category: "", url: "", type: "image" },
...
function ItemModal({
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, fieldKey: string) => {
    ...
    if (!isCloudinaryConfigured) { alert("Cloudinary not configured. Please add VITE_CLOUDINARY_CLOUD_NAME and VITE_CLOUDINARY_UPLOAD_PRESET to your .env file."); return; }
    setUploading(true);
    try {
      if (files.length === 1) {
        // Single image upload (staff photo, event cover, topper photo)
        const result = await uploadToCloudinary(files[0], "school");
        set(fieldKey, result.secure_url);
      } else {
        // Multiple images — upload all and set comma-separated URLs
        const urls: string[] = [];
        for (const file of Array.from(files)) {
          const result = await uploadToCloudinary(file, "school");
          urls.push(result.secure_url);
        }
        // For gallery URL field, set first image; for others join by comma
        set(fieldKey, urls[0]);
        if (urls.length > 1) {
          alert(`${urls.length} images uploaded! Only first image is set in this field. For gallery, add each image separately.`);
        }
      }
    } catch (err: any) {
      const msg = err?.message ?? "Unknown error";
      alert(`Upload failed: ${msg}\n\nCheck:\n• Cloudinary Cloud Name & Upload Preset are correct\n• Upload preset is set to "Unsigned" in Cloudinary dashboard`);
    } finally {
      setUploading(false);
...
function SectionManager({ sectionKey, items, onBack }: { sectionKey: Section; items: any[]; onBack: () => void }) {
  ...
  const getSubtitle = (item: any) => {
    ...
    if (sectionKey === "gallery") return item.category ?? "";
  ...
  const getThumb = (item: any) =>
    item.photo ?? item.coverImage ?? item.url ?? null;
...
export default function AdminDashboard() {
  ...
  if (configured && !user) return <Navigate to="/admin
