...
function ImagePositionEditor({
  ...
  return (
    <div className="mt-2 p-3 rounded-xl border bg-muted/30 space-y-3">
      ...
      {/* Preview */}
      <div className={`mx-auto ${shape === "circle" ? "w-24 h-24 rounded-full" : "w-24 h-32 rounded-lg"} overflow-hidden border-2 border-primary/30 bg-muted`}>
        <img
          src={url}
          alt="preview"
          style={{
        ...
        <div className="grid grid-cols-3 gap-1 w-fit mx-auto">
          {positions.map((pos) => (
            <button
              key={pos}
              type="button"
              onClick={() => onChange(pos, objectFit, zoom)}
              className={`w-8 h-8 rounded text-sm flex items-center justify-center transition-colors font-mono
                ${objectPosition === pos ? "bg-primary text-primary-foreground" : "bg-card border hover:bg-muted"}`}
              title={pos}
            >
              {posLabels[pos]}
            </button>
